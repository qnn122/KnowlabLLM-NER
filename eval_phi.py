...
def eval_subject(
    ...
    **kwargs,
    ...
    all_probs = {"prob_A": [], "prob_B": [], "prob_C": []}
    if args.debug:
        print(f"few_shot_prompt: {few_shot_prompt}")

    choices_ids = torch.tensor(
        tokenizer(["A", "B", "C"], add_special_tokens=False)["input_ids"]).flatten().unsqueeze(0).to(model.device)
    ...
    for i in tqdm(idx_list):
        ...
        for i in range(len(probs)):
            ...
            pred = {0: "A", 1: "B", 2: "C"}[np.argmax(probs[i])]
...
def cal_mmlu(res, save_result_dir):
    ...
    for class_ in SUBJECTS:
        acc_dict[class_] = sum(res[class_]) / len(res[class_]) * 100
        acc_sum += sum(res[class_])
        cnt += len(res[class_])

    acc_dict['AVERAGE ACC']=acc_sum / cnt * 100
    
    temp_df=pd.DataFrame([acc_dict])
    temp_df.to_csv(
        os.path.join(save_result_dir, "accuracy_result.csv"),
        encoding="utf-8",
        index=False,
    )
...
def main(args):
    ...
    for subject_name in tqdm(SUBJECTS):
        ...
        dev_df = pd.read_csv(
            dev_file_path, names=["question", "A", "B", "C", "answer"], sep='\t'
        )
        test_df = pd.read_csv(
            test_file_path, names=["question", "A", "B", "C", "answer"], sep='\t'
        )
        ...
        dev_result[subject_name] = score
    #cal_mmlu(dev_result, save_result_dir=args.output_result_dir)
